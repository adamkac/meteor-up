...
exports.read = function() {
  ...
  if(fs.existsSync(mupJsonPath)) {
    ...
    if(typeof mupJson.setupJelasticAPI === "undefined") {
      mupJson.setupJelasticAPI = false;
    }
    ...
    if(!mupJson.servers || mupJson.servers.length == 0) {
      ...
      mupJson.servers.forEach(function(server) {
        ...
        if(!server.host && server.os != "jelastic") {
        ...
        } else if(server.os == "jelastic") {
          var hasJava = hasbin.all.sync(['java']);
          var hasJelasticApi = fs.existsSync(rewriteHome('~/jelastic/'));
          
          if(!hasJava){
            mupErrorLog('You do not have Java installed. Required version is >= 1.7. More info: https://docs.jelastic.com/cli');
          }
          if(!hasJelasticApi){
            mupErrorLog('You do not have JelasticCli installed. More info: https://docs.jelastic.com/cli');
          }
          if(!server.envName){
            mupErrorLog('Jelastic envName not defined. More informations: https://docs.jelastic.com/api-overview');
          }
          if(!server.platformUrl){
            mupErrorLog('Jelastic platformUrl not defined. Please fill it with eq. app.unicloud.pl. More informations: https://docs.jelastic.com/api-overview');
          }
        }
