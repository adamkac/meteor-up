...
function buildApp(appPath, meteorBinary, buildLocaltion, config, callback) {
  buildMeteorApp(appPath, meteorBinary, buildLocaltion, config, function(code) {
    if(code == 0) {
...
function buildMeteorApp(appPath, meteorBinary, buildLocaltion, config, callback) {
  var executable = meteorBinary;
  console.log(config);
  console.log(buildLocaltion);
  var args = [
    "build", "--directory", buildLocaltion, 
    "--server", config.env.ROOT_URL
  ];
